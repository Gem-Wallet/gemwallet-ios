...
clean:
	rm -rf $(vendor_path)
	rm -rf ./Pods/*
	@xcodebuild -quiet -disableAutomaticPackageResolution -workspace GemWallet.xcworkspace -scheme GemWallet -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 12,OS=latest' clean
...
test15:
	@xcodebuild -disableAutomaticPackageResolution -workspace GemWallet.xcworkspace -scheme GemWallet -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 12,OS=15.4' test | $(beautify_cmd)

test14:
	@xcodebuild -disableAutomaticPackageResolution -workspace GemWallet.xcworkspace -scheme GemWallet -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 12,OS=14.5' test | $(beautify_cmd)

test_latest:
	@xcodebuild -disableAutomaticPackageResolution -workspace GemWallet.xcworkspace -scheme GemWallet -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 12,OS=latest' test | $(beautify_cmd)

test: test_latest

build_and_run_booted:
	#The simulator "name" specified doesn't matter
	@xcrun xcodebuild -disableAutomaticPackageResolution -scheme GemWallet -workspace GemWallet.xcworkspace -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 12 Pro,OS=15.4' -derivedDataPath ./build | $(beautify_cmd)
	@xcrun simctl install booted ./build/Build/Products/Debug-iphonesimulator/GemWallet.app
	@xcrun simctl launch booted com.stormbird.gemwallet
...
update_chains_file:
	...
	@mv scripts/chains.json.zip GemWallet/Rpc\ Network/chains.zip
	@rm -f ./scripts/chains.json
